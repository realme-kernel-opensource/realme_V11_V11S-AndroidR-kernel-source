 ...
 */
static int gc_failure_swap_seq_show(struct seq_file *seq, void *offset)
{
        struct super_block *sb = seq->private;
        struct f2fs_sb_info *sbi = F2FS_SB(sb);
...
static int gc_failure_swap_open(struct inode *inode, struct file *file)
{
        return single_open(file, gc_failure_swap_seq_show, PDE_DATA(inode));
}

static const struct file_operations f2fs_seq_gc_failure_swap_fops = {
        .open = gc_failure_swap_open,
        .read = seq_read,
        .llseek = seq_lseek,
...
int f2fs_register_sysfs(struct f2fs_sb_info *sbi)
{
	...
	if (sbi->s_proc) {
		 ...
		 * 2019/09/11, add f2fs frag_score and undiscard_score
		 */
		proc_create_data("gc_failure_swap", S_IRUGO, sbi->s_proc,
                                 &f2fs_seq_gc_failure_swap_fops, sb);
...
void f2fs_unregister_sysfs(struct f2fs_sb_info *sbi)
{
	if (sbi->s_proc) {
		...
		remove_proc_entry("gc_failure_swap", sbi->s_proc);
		/* VENDOR_EDIT shifei.ge@TECH.Storage.FS
		 * 2019-08-14, add for oDiscard
		 */
