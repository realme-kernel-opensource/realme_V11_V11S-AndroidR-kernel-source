...
static bool oplus_is_power_off_charging(void);
...
static void oplus_chg_check_tbatt_normal_status(struct oplus_chg_chip *chip)
{
	...
	if (chip->tbatt_status == BATTERY_STATUS__NORMAL) {
	...
	else if ((chip->tbatt_status == BATTERY_STATUS__COOL_TEMP) || (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP)) {
		chip->limits.normal_phase1_bat_decidegc = chip->tbatt_normal_anti_shake_bound.phase1_bound;
		chip->limits.normal_phase2_bat_decidegc = chip->tbatt_normal_anti_shake_bound.phase2_bound;
		chip->limits.normal_phase3_bat_decidegc = chip->tbatt_normal_anti_shake_bound.phase3_bound;
		chip->limits.normal_phase4_bat_decidegc = chip->tbatt_normal_anti_shake_bound.phase4_bound;
		...
		chg_debug("reset_limits_normal_phase, [chip->tbatt_status = %d chip->tbatt_normal_status = %d] [%d %d %d %d %d %d]\n",
		chip->tbatt_status, chip->tbatt_normal_status,
...
#endif
	chg_debug("oplus_chg_variables_init 1\n");
...
//	chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->ffc_temp_status = FFC_TEMP_STATUS__NORMAL;
	chip->vooc_temp_status = VOOC_TEMP_STATUS__NORMAL;
	chg_debug("oplus_chg_variables_init 4\n");
	...
	chip->cool_down_force_5v = false;
	chip->chg_ctrl_by_cool_down = false;
	chg_debug("oplus_chg_variables_init 5\n");
}
...
#define CURRENT_LIMIT_COUNT 6
static const int cool_down_current_limit_normal[CURRENT_LIMIT_COUNT] = {500, 900, 1200, 1500, 2000, 1500};
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (val > CURRENT_LIMIT_COUNT) {
		chip->limits.input_current_charger_ma = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->limits.pd_input_current_charger_ma = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->limits.qc_input_current_charger_ma = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->limits.input_current_vooc_ma_high = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->limits.input_current_vooc_ma_warm = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->limits.input_current_vooc_ma_normal = cool_down_current_limit_normal[CURRENT_LIMIT_COUNT - 1];
		chip->cool_down_done = true;
	} else {
		...
		chip->limits.input_current_vooc_ma_high = cool_down_current_limit_normal[val - 1];
		chip->limits.input_current_vooc_ma_warm = cool_down_current_limit_normal[val - 1];
		chip->limits.input_current_vooc_ma_normal = cool_down_current_limit_normal[val - 1];
		chip->cool_down_done = true;
	}
