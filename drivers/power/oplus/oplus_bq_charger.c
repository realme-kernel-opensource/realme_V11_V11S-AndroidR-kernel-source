...
#define OPLUS_CHG_UPDATE_INIT_DELAY        round_jiffies_relative(msecs_to_jiffies(500))        /* first run after init 10s */
#define OPLUS_CHG_UPDATE_INTERVAL        round_jiffies_relative(msecs_to_jiffies(OPLUS_CHG_UPDATE_INTERVAL_SEC*1000))        /* update cycle 5s */
        ...
        union power_supply_propval *val)
{
		...
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oplus_usb_get_property:oplus_chg_chip is NULL\n");
			pr_err("oplus_usb_get_property: property %d\n", psp);
			ret = -EINVAL;
			val->intval = 0;
			return ret;
		}
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oplus_usb_set_property: oplus_chg_chip is NULL\n");
			ret = -EINVAL;
			return ret;
		}
#endif /*CONFIG_OPLUS_CHARGER_MTK6769*/
        ...
        union power_supply_propval *val)
{
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oplus_ac_get_property: oplus_chg_chip is NULL\n");
			pr_err("oplus_ac_get_property: property %d\n", psp);
			ret = -EINVAL;
			val->intval = 0;
			return ret;
		}
        ...
        case POWER_SUPPLY_PROP_FAST2NORMAL_CHG:
                if (oplus_vooc_get_fastchg_to_normal() == true || oplus_vooc_get_fastchg_to_warm() == true
                        || oplus_vooc_get_btb_temp_over() == true || oplus_vooc_get_fastchg_low_temp_full() == true) {
                        val->intval = 1;
                } else {
                        val->intval = 0;
                }
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oplus_batery_set_property: oplus_chg_chip is NULL\n");
			ret = -EINVAL;
			return ret;
		}
#endif /*CONFIG_OPLUS_CHARGER_MTK6769*/
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
		if (IS_ERR_OR_NULL(chip)) {
			pr_err("oplus_battery_get_property: oplus_chg_chip is NULL\n");
			pr_err("oplus_battery_get_property: %d\n", psp);
			val->intval = 0;
			ret = -EINVAL;
			return ret;
		}
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
                } else if (!chip->batt_full && (POWER_SUPPLY_STATUS_FULL == chip->prop_status)) {
                        val->intval = POWER_SUPPLY_STATUS_CHARGING;
#endif /* CONFIG_OPLUS_CHARGER_MTK6769 */
                } else {
...
#else
	chip->suspend_ws = wakeup_source_register(chip->dev, "battery suspend wakelock");
#endif
}
...
/*12~16 C*/
        ...
        if (rc < 0) {
                chip->limits.qc_temp_little_cool_fastchg_current_ma_high = chip->limits.temp_little_cool_fastchg_current_ma_high;
        }
#endif
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
void oplus_chg_usb_variables_reset(struct oplus_chg_chip *chip)
{
...
#endif /*CONFIG_OPLUS_CHARGER_MTK6769*/
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
static void oplus_chg_set_charger_vol_9v(struct oplus_chg_chip *chip,bool en)
{
	if(!chip->chg_ops->oplus_chg_set_high_vbus)
		return;
	if(!chip->chg_ops->get_charger_subtype)
		return;
	
	 charger_xlog_printk(CHG_LOG_CRTI, "oplus_chg_set_charger_vol_9v = %d\n", en);
	if(chip->chg_ops->get_charger_subtype() !=CHARGER_SUBTYPE_DEFAULT)
			chip->chg_ops->oplus_chg_set_high_vbus(en);
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
		...
		if(chip->charger_type == POWER_SUPPLY_TYPE_USB) {
			oplus_chg_usb_variables_reset(chip);
		}
		...
		if((chip->is_double_charger_support)&&(chip->chg_ops->get_charger_subtype() !=CHARGER_SUBTYPE_DEFAULT)){
			...
			if(chip->tbatt_status==BATTERY_STATUS__NORMAL){
				if(chip->jeita_sm == BATT_TEMP_EXTEND_T0_TO_T1){
					...
					voter = TBATT;
				}else if(chip->jeita_sm == BATT_TEMP_EXTEND_T5_TO_T6){
					charging_current=chip->limits.batt_temp_extend_t5_to_t6_ichg;
					voter = TBATT;
				}else if(chip->jeita_sm == BATT_TEMP_EXTEND_T6_TO_T7){
					charging_current=chip->limits.batt_temp_extend_t6_to_t7_ichg;
					voter = TBATT;
				}
			}

			if(chip->jeita_sm>=BATT_TEMP_EXTEND_T6_TO_T7 ||
					(chip->tbatt_status >= BATTERY_STATUS__COLD_TEMP && chip->tbatt_status <= BATTERY_STATUS__LITTLE_COLD_TEMP)){
				oplus_chg_set_charger_vol_9v(chip,0);
			}else{
				oplus_chg_set_charger_vol_9v(chip,1);
			}
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on) && (charging_current > chip->limits.temp_warm_fastchg_current_ma_led_on)) {
		if (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP || chip->jeita_sm == BATT_TEMP_EXTEND_T6_TO_T7) {
...
#define BATT_TEMP_EXTEND_T2 300
#define BATT_TEMP_EXTEND_T2_MINUS_X 280
#define BATT_TEMP_EXTEND_T3 340
#define BATT_TEMP_EXTEND_T3_MINUS_X 320
#define BATT_TEMP_EXTEND_T4 370
#define BATT_TEMP_EXTEND_T4_MINUS_X 350
#define BATT_TEMP_EXTEND_T5 400
#define BATT_TEMP_EXTEND_T5_MINUS_X 380
#define BATT_TEMP_EXTEND_T6 420
#define BATT_TEMP_EXTEND_T6_MINUS_X 410
#define BATT_TEMP_EXTEND_T7 450
#define BATT_TEMP_EXTEND_T7_MINUS_X 430
...
#define BATT_TEMP_EXTEND_T3_TO_T4_ICHG 1750
...
#define BATT_TEMP_EXTEND_T5_TO_T6_ICHG 1600 //need vbus to 5v
#define BATT_TEMP_EXTEND_T6_TO_T7_ICHG 1600 //need vbus to 5v
...
static void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_calling) && (chip->calling_on) && (current_limit > chip->limits.input_current_calling_ma)) {
	 ...
	 if (chip->chg_ctrl_by_vooc && chip->vbatt_num == 2
		&& oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC && oplus_vooc_get_fastchg_started() == true) {
		if(chip->led_on) {
			current_limit = 1800;
		} else {
...
#endif /*CONFIG_OPLUS_CHARGER_MTK6769*/
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
static void oplus_chg_check_thermal_on_ichging(struct oplus_chg_chip *chip)
{
	OPLUS_CHG_AP_TEMP_STAT pre_ap_sm = chip->ap_sm;
	OPLUS_CHG_BATT_TEMP_EXTEND_STAT pre_jeita_sm =  chip->jeita_sm;
	...
	if(chip->is_double_charger_support){
		if (batt_temp >= BATT_TEMP_EXTEND_T7) {
			chip->jeita_sm = BATT_TEMP_EXTEND_ABOVE_T7;
		} else if (batt_temp >= BATT_TEMP_EXTEND_T6) {
			if ((BATT_TEMP_EXTEND_ABOVE_T7 != chip->jeita_sm )
				|| (batt_temp <= BATT_TEMP_EXTEND_T7_MINUS_X)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T6_TO_T7;
			}
		} else if (batt_temp >= BATT_TEMP_EXTEND_T5) {
			if ((BATT_TEMP_EXTEND_T6_TO_T7 != chip->jeita_sm )
				|| (batt_temp <= BATT_TEMP_EXTEND_T6_MINUS_X)) {
				chip->jeita_sm	= BATT_TEMP_EXTEND_T5_TO_T6;
			}
		}else if (batt_temp >= BATT_TEMP_EXTEND_T4) {
			if ((BATT_TEMP_EXTEND_T5_TO_T6 != chip->jeita_sm )
...
static void oplus_chg_check_vbatt_higher_than_4000mv(struct oplus_chg_chip *chip)
{
        static bool vol_high_pre = false;
        static int lower_count = 0, higher_count = 0;
        static int tbatt_status_pre = BATTERY_STATUS__NORMAL;
...
static void oplus_chg_check_vbatt_higher_than_4400mv(struct oplus_chg_chip *chip)
{
        static bool vol_high_pre = false;
        static int lower_count = 0, higher_count = 0;
        static int tbatt_status_pre = BATTERY_STATUS__NORMAL;
...
static void oplus_chg_voter_charging_stop(struct oplus_chg_chip *chip, OPLUS_CHG_STOP_VOTER voter)
{
...
static bool oplus_chg_check_time_is_good(struct oplus_chg_chip *chip)
{
	...
	if(get_eng_version() == 1) {
		chip->chging_over_time = false;
		printk("oplus_chg_check_time_is_good_aging_mode\n");
		return true;
	}
//endif
...
static void oplus_chg_check_led_on_ichging(struct oplus_chg_chip *chip)
{
	if (chip->led_on_change || (chip->led_on && chip->led_temp_change)) {
		...
		if (chip->chg_ctrl_by_vooc && chip->vbatt_num == 2) {		
			if (oplus_vooc_get_fastchg_started() == true && oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC) {		
				return;
			}
			if (oplus_vooc_get_allow_reading() == true || oplus_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC) {
				oplus_chg_set_charging_current(chip);
				oplus_chg_set_input_current_limit(chip);
			}
		} else {
...
static void oplus_chg_check_vooc_temp_status(struct oplus_chg_chip *chip)
{
        ...
        if (chip->vbatt_num != 2 || oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC || oplus_vooc_get_fastchg_started() == false) {
...
//        chip->anti_shake_bound.overtemp_bound = chip->limits.overtemp_bat_decidegc;
	...
	chip->limits.batt_temp_extend_t5_to_t6_ichg = BATT_TEMP_EXTEND_T5_TO_T6_ICHG;
	chip->limits.batt_temp_extend_t6_to_t7_ichg = BATT_TEMP_EXTEND_T6_TO_T7_ICHG;
	...
	if(ap_temp_debug == 65535)
        ...
        if (ui_soc_cp_flag == 0) {
                if ((chip->soc < 0 || chip->soc > 100) && retry_counts < RETRY_COUNTS) {
                        charger_xlog_printk(CHG_LOG_CRTI, "[Battery]oplus_chg_get_battery_data,chip->soc[%d],retry_counts[%d]\n", chip->soc, retry_counts);
                        retry_counts++;
                        chip->soc = 50;
                        goto next;
                }
                ...
                charger_xlog_printk(CHG_LOG_CRTI, "[Battery]oplus_chg_get_battery_data,soc_load = %d,soc = %d\n", soc_load, chip->soc);
        }
        next:
        return;
}
...
static void oplus_chg_check_aicl_input_limit(struct oplus_chg_chip *chip)
{
        ...
        if (chip->charging_state == CHARGING_STATUS_FAIL || chip->batt_full == true
                || ((chip->tbatt_status != BATTERY_STATUS__NORMAL) && (chip->tbatt_status != BATTERY_STATUS__LITTLE_COOL_TEMP))
                || ((chip->ui_soc > 85) && (chip->pmic_spmi.aicl_suspend == false)) || oplus_vooc_get_fastchg_started() == true) {
                aicl_delay_count = 0;
                return;
        }
...
static void oplus_chg_protection_check(struct oplus_chg_chip *chip)
{
		...
		oplus_chg_check_vbatt_higher_than_4400mv(chip);
		oplus_chg_check_vbatt_higher_than_4000mv(chip);
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
	oplus_chg_check_thermal_on_ichging(chip);
#endif /*CONFIG_OPLUS_CHARGER_MTK6769*/
}
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
        static int counts_n = 0;
        static bool almost_full = false;
#endif
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
				if (chip->icharging < 0 && (false == almost_full)
					&& ((chip->icharging * -1) <= chip->limits.iterm_ma + 300)
					&& chip->is_double_charger_support) {
					counts_n++;
					if (counts_n > chip->sw_full_count) {
						counts_n = 0;
						almost_full = true;
						chip->sub_chg_ops->charging_disable();
					}
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
        ...
        if (chip->chg_ctrl_by_vooc) {
		if (oplus_vooc_get_fastchg_ing() == true && oplus_vooc_get_fast_chg_type() != CHARGER_SUBTYPE_FASTCHG_VOOC)
			return;
        } else {
		if (oplus_vooc_get_fastchg_ing() == true) {
			return;
        ...
        if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL) || oplus_chg_check_vbatt_is_full_by_sw(chip)) {
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
			chip->limits.temp_little_cool_fastchg_current_ma_high = chip->limits.qc_temp_little_cool_fastchg_current_ma_high;
#endif
...
#ifdef CONFIG_OPLUS_CHARGER_MTK6769
void oplus_pe20_check(struct oplus_chg_chip *chip)
{
	...
	if (!chip->chg_ops->check_chrdet_status || !chip->chg_ops->oplus_chg_get_pe20_type
		||!chip->chg_ops->oplus_chg_reset_pe20||!chip->chg_ops->get_charger_subtype)
		return;
	...
	if (chip->chg_ops->check_chrdet_status()){
		if(charger_type==POWER_SUPPLY_TYPE_USB_DCP){
			if((pe20_check_count<1)&&(chip->chg_ops->get_charger_subtype()==CHARGER_SUBTYPE_DEFAULT)){
				...
				if(diff.tv_sec < 2){
					chg_err("oplus_pe20_check:timer:%ld:%ld\n",diff.tv_sec,diff.tv_nsec);
					return;
				}
				
				if(chip->chg_ops->oplus_chg_get_pe20_type()== false){
					pe20_check_count++;
				}else{
					plug_flag=1;
					chg_err("oplus_pe20_check:ok\n");
				}
				oplus_chg_set_charging_current(chip);
				oplus_chg_set_input_current_limit(chip);
			}
		...
		if(plug_flag){
			plug_flag=0;
			chg_err("oplus_pe20_check:reset\n");
			chip->chg_ops->oplus_chg_reset_pe20();
		}
	}
}

static void oplus_chg_pe20_config(struct oplus_chg_chip *chip)
{
	int ret = 0;
	int charger_type;
	...
	if (chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_PE20) {
		ret = chip->chg_ops->oplus_chg_pe20_setup();
		if (ret >= 0) {
...
static void oplus_chg_pe20_to_normal(struct oplus_chg_chip *chip)
{
	int ret = 0;
...
int oplus_chg_get_charger_exist(void)
{
        if (!g_charger_chip) {
                return -1;
        } else {
