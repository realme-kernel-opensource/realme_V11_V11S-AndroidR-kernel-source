...
extern struct oplus_chg_operations * svooc_oplus_get_chg_ops(void);
#endif
...
int oplus_mt6360_disable_charging(void);
static int oplus_mt6360_float_voltage_write(int vflaot_mv);
int oplus_mt6360_suspend_charger(void);
		...
		/*if (g_oplus_chip->dual_charger_support) {
			if (g_oplus_chip->camera_on == 1 && g_oplus_chip->charger_exist) {
				...
				//if(oplus_vooc_get_fastchg_ing() == true)
					...
					//oplus_chg_set_pd_config();//oplus_mt6360_pd_setup();
				}
			}
		}*/
...
#if 1
/*zhangchao@ODM.HQ.BSP.CHG 2020/04/22 modify for sala_A charging bring up*/
bool oplus_mt6360_get_vbus_status(void)
#else
...
static void enter_ship_mode_function(struct oplus_chg_chip *chip)
{
	...
	if (chip->enable_shipmode) {
		...
		if((get_project() == 19661) && ((get_Operator_Version() == 111) || (get_Operator_Version() == 112) || (get_Operator_Version() == 113) || (get_Operator_Version() == 114) || (g_cphy_dphy_gpio_value == 1))) {
			printk(KERN_ERR "[OPLUS_CHG][%s]: MTK mt6360 !\n", __func__);
			...
			printk(KERN_ERR "[OPLUS_CHG][%s]: 1878 HW chip !\n", __func__);
...
extern int oplus_usb_ntc_adc1_read(void);
extern int oplus_usb_ntc_adc2_read(void);
extern int oplus_flashlight_ntc_adc5_read(void);
extern int oplus_charger_ntc_adc4_read(void);
...
static void oplus_get_usbtemp_volt(struct oplus_chg_chip *chip)
{
	...
	usbtemp_volt = oplus_usb_ntc_adc1_read();//get the typec tem adc volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = USB_25C_VOLT;
	}
	chip->usbtemp_volt_r = usbtemp_volt;

	usbtemp_volt = oplus_usb_ntc_adc2_read();//get the typec tem adc2 volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = USB_25C_VOLT;
	}
...
static void get_usb_temp(struct oplus_chg_chip *chip)
{
	...
	//printk(KERN_ERR "[OPLUS_CHG][%s]:usbtemp_volt: %d, %d,usbtemp:%d,%d\n",__func__,chip->usbtemp_volt_r, chip->usbtemp_volt_l,chip->usb_temp_r,chip->usb_temp_l);
	return;
}
#ifdef CONFIG_OPLUS_HQ_CHARGER
/*wangtao@ODM.HQ.BSP.CHG 2020/05/08 modify flashlight_temp*/
void oplus_flashlight_temp_check(struct oplus_chg_chip *chip)
{
	int usbtemp_volt = 0;
	int i = 0;
	int notify = 0; 
	...
	usbtemp_volt = oplus_flashlight_ntc_adc5_read();//get flashlight volt
	if (usbtemp_volt <= 0) {
		usbtemp_volt = USB_25C_VOLT;
	}
	...
	//printk(KERN_ERR "[OPLUS_CHG][%s]: flashlight_temp = %d,notify_code =%d\n", __func__,chip->flashlight_temp,chip->notify_code);
	return;
}
#endif
					...
					int *temp)
{
	...
	charger_ntc_volt= oplus_charger_ntc_adc4_read();
		...
		if ((chip->usb_temp_r >= USB_57C && chip->usb_temp_r <= USB_100C)
			...
			if (count <= time_count) {
				...
				if(count == 0){
					...
					//printk(KERN_ERR "[OPLUS_CHG][%s]:last_usbtemp:%d,%d\n",__func__,last_usb_temp_r,last_usb_temp_l);
				}
				#endif
...
{
	#ifdef ODM_HQ_EDIT
	/*liuting@ODM.HQ.BSP.CHG 2020/04/22 modify for factory otg node*/
	...
	if (pinfo != NULL && pinfo->tcpc != NULL) {
		...
		if(get_vbatt_num() == 2) {
			printk(KERN_ERR "[OPLUS_CHG][%s]: otg switch[%d],g_oplus_chip->charger_exist =%d\n", __func__, value,g_oplus_chip->charger_exist);
			if(!g_oplus_chip->charger_exist) {
				if(value){
					printk(KERN_ERR "[OPLUS_CHG][%s]: otg_enable[%d]\n", __func__, value);
					g_oplus_chip->chg_ops->otg_enable(); 
				}
				else{
					printk(KERN_ERR "[OPLUS_CHG][%s]: otg_disable[%d]\n", __func__, value);
...
//====================================================================//
#ifdef VENDOR_EDIT
bool oplus_mt6360_get_pd_type(void)
{
	if (pinfo != NULL) {
		printk("oplus_mt6360_get_pd_type pinfo->pd_type =%d,oplus_mtk_pdc_check_charger(pinfo)=%d\n",pinfo->pd_type,mtk_pdc_check_charger(pinfo));
		//pd_notify = pinfo->pd_type; temp modify
...
int oplus_chg_set_pd_config(void)
{
    ...
    if (g_oplus_chip->vbatt_num == 2) {
        if((oplus_chg_show_vooc_logo_ornot() == 0) && (oplus_vooc_get_fastchg_started() == false)){
            if (g_oplus_chip->limits.vbatt_pdqc_to_5v_thr > 0 && g_oplus_chip->charger_volt > 7500 && g_oplus_chip->batt_volt > g_oplus_chip->limits.vbatt_pdqc_to_5v_thr) {
                ...
                vbus_mv = VBUS_5V;
                ibus_ma = IBUS_2A;
            } else if (g_oplus_chip->batt_volt < g_oplus_chip->limits.vbatt_pdqc_to_9v_thr) {

                oplus_chg_suspend_charger();
                oplus_chg_config_charger_vsys_threshold(0x02);//set Vsys Skip threshold 101%
                oplus_chg_enable_burst_mode(false);
                vbus_mv = VBUS_9V;
                ibus_ma = IBUS_2A;
            }
        }
    }else if (g_oplus_chip->vbatt_num == 1){
        vbus_mv = VBUS_5V;
        ibus_ma = IBUS_3A;
...
int oplus_chg_set_qc_config_forsvooc(void)
{
	...
	if (!chip->calling_on && !chip->camera_on && chip->charger_volt < 6500 && chip->soc < 90
		...
		if (chip->charger_volt > 7500 &&
			(chip->calling_on || chip->camera_on || chip->soc >= 90
...
/*zhangchao@ODM.HQ.BSP.CHG 2020/04/22 modify for sala_A charging bring up*/
	if ((get_project() == 20682)) {
		oplus_chip->vbatt_num = is_sala_a_project();
		chg_err("[oplus_chg_init] vbatt_num = %d\n",oplus_chip->vbatt_num);
		chg_err("[oplus_chg_init] gauge = %d ,vooc = %d ,adapter = %d\n",oplus_gauge_check_chip_is_null(),oplus_vooc_check_chip_is_null(),oplus_adapter_check_chip_is_null());
	}
#endif
	if (oplus_chip->vbatt_num == 1) {
		...
		if ((get_project() == 20682)) {
			if (oplus_chip->vbatt_num == 2) {
				oplus_chip->chg_ops = (svooc_oplus_get_chg_ops());
			}
		}
		#endif
	...
	printk(KERN_ERR "[OPLUS_CHG][%s]: zhangchao mtk_charger_probe sucess\n", __func__);
	#endif
