...
static int g_rap_pull_up_r0 = 0;/*pull_up_r */
	...
	int idx, int *tchg_min,	int *tchg_max)
{
	...
	if (info != NULL) {
		...
		chargeric_temp = oplus_mt6360_get_tchg(tchg_min, tchg_max);

		pdata->junction_temp_min = chargeric_temp;
		pdata->junction_temp_max = chargeric_temp;
		*tchg_min = chargeric_temp;
		*tchg_max = chargeric_temp;

		printk("wenboyu chargeric_temp = %d\n", chargeric_temp);
		return 0;
	}
	return -EBUSY;
}
...
/*Ping.Wu@BSP.CHG.Basic 2021/01/12 add for pull-up start*/
        if (of_property_read_u32(np, "qcom,g_rap_pull_up_r0", &val) >= 0) {
		g_rap_pull_up_r0 = val;
	} else {
		chg_err("use g_rap_pull_up_r0 100k!\n");
		g_rap_pull_up_r0 = 100000;
	}
        printk("wp:g_rap_pull_up_r0 = %d\n", g_rap_pull_up_r0);
/*Ping.Wu@BSP.CHG.Basic 2021/01/12 add for pull-up end*/

#ifdef OPLUS_FEATURE_CHG_BASIC
/*Boyu.Wen@BSP.CHG.Basic 2020/11/2 add for pd*/
	...
	chg_err("step1_time: %d, step1_current: %d, step2_time: %d, step2_current: %d, step3_current: %d, pd_not_support: %d\n",
		info->data.step1_time, info->data.step1_current_ma, info->data.step2_time, info->data.step2_current_ma, info->data.step3_current_ma, info->data.pd_not_support);
...
static void oplus_mt6360_dump_registers(void)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		} else if (bq25601d_is_detected() == true) {
...
static int oplus_mt6360_charging_current_write_fast(int chg_curr)
{
	...
	if (pinfo->data.dual_charger_support) {
		...
		if ((bq25910_is_detected() == false)&&(bq25601d_is_detected() == false)) {
...
static __s16 oplus_ts_ch_volt_to_temp(__u32 dwvolt)
{
	...
	const int g_rap_pull_up_r = g_rap_pull_up_r0; /* 390K, pull up resister */
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/*Boyu.Wen@BSP.CHG.Basic 2020/11/2 add for charge*/
...
#ifdef OPLUS_FEATURE_CHG_BASIC
/*Boyu.Wen@BSP.CHG.Basic 2020/11/2 add for charge*/
		if(info->data.dual_charger_support == 0)
