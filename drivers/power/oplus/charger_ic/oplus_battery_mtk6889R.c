		...
		if ((chip->usb_temp_r >= USB_57C && chip->usb_temp_r < USB_100C)
			...
			if (count_r >= RETRY_COUNT || count_l >= RETRY_COUNT) {
				if (!IS_ERR_OR_NULL(chip->normalchg_gpio.dischg_enable)) {
					...
					chg_err("check for CONFIG_HIGH_TEMP_VERSION\n");
			...
			count_l = 0;
			count = 0;
		} else if (((chip->usb_temp_r  - chip->temperature/10) > 18 && chip->usb_temp_r >= USB_20C && chip->usb_temp_r < USB_100C)
				|| ((chip->usb_temp_l  - chip->temperature/10) > 18 && chip->usb_temp_l >= USB_20C && chip->usb_temp_l < USB_100C)) {
			if (count <= time_count) {
				if (count == 0) {
