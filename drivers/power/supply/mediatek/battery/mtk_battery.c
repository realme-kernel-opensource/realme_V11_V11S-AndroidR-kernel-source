...
int battery_temp_01_precision = 0;
	...
	if (update == true || pre_bat_temperature_val == -1) {
		...
		if (bat_temperature_volt != 0) {
			...
			bm_err("[force_get_tbat_internal] tbat_precise bat_temperature_val:%d\n",
				bat_temperature_val);
			battery_temp_01_precision = bat_temperature_val;
		}
...
int oplus_battery_get_bat_temperature(void)
{
   ...
   if (is_battery_init_done()) {
       return battery_temp_01_precision;
   }
   else {
       return -1270;
   }
...
try_again:
	battery_type = battery_type_check();
        if((get_project()== 20015)||(get_project()== 20016)||(get_project()== 20108)||(get_project()== 20109)){
			if (battery_type == BAT_TYPE__COS_4450mV || battery_type == BAT_TYPE__LIW_4450mV) {
				return true;
			} else {
			if (retry_flag == 0) {
				retry_flag = 1;
		...
		}else if((get_project()== 20651)||(get_project()== 20652)||(get_project()== 21637)||(get_project()== 21638)){
			if (battery_type == BAT_TYPE__COS_4450mV || battery_type == BAT_TYPE__LIW_4450mV || battery_type == BAT_TYPE__ATL_4450mV) {
				return true;
			} else {
			if (retry_flag == 0) {
				retry_flag = 1;
       ...
       }else if((get_project()== 20608)||(get_project()== 20609)||(get_project()== 0x2060A)||(get_project()== 0x2060B)||(get_project()== 0x206FF)||(get_project()== 0x206F0)||(get_project()== 20796)||(get_project()== 20795) || (get_project()== 0x2070C)||(get_project()== 0x2070E)||(get_project()== 0x2070B)) {
               if (battery_type == BAT_TYPE__COS_4450mV || battery_type == BAT_TYPE__ATL_4450mV){
           	     return true;
		} else {
			if (retry_flag == 0) {
				retry_flag = 1;
