...
struct mt6833_log_helper {
	short cur;
	short prev;
	struct mt6833_spm_wake_status *wakesrc;
};
struct mt6833_log_helper mt6833_logger_help = {
	.wakesrc = &mt6833_wake,
	.cur = 0,
	.prev = 0,
};

static char *mt6833_spm_cond_cg_str[PLAT_SPM_COND_MAX] = {
...
const char *mt6833_wakesrc_str[32] = {
	[0] = " R12_PCM_TIMER",
...
struct spm_wakesrc_irq_list mt6833_spm_wakesrc_irqs[] = {
	...
	{ WAKE_SRC_STA1_CONN2AP_SPM_WAKEUP_B, "mediatek,mt6833-consys", 0, 0},
	/* conn2ap_sw_irq */
	{ WAKE_SRC_STA1_CONN2AP_SPM_WAKEUP_B, "mediatek,mt6833-consys", 2, 0},
	/* CCIF_AP_DATA */
...
#define plat_mmio_read(offset)	__raw_readl(mt6833_spm_base + offset)
u64 ap_pd_count;
u64 ap_slp_duration;
...
#define IRQ_NUMBER	\
(sizeof(mt6833_spm_wakesrc_irqs)/sizeof(struct spm_wakesrc_irq_list))
static void mt6833_get_spm_wakesrc_irq(void)
{
	int i;
	struct device_node *node = NULL;

	for (i = 0; i < IRQ_NUMBER; i++) {
		if (mt6833_spm_wakesrc_irqs[i].name == NULL)
			continue;

		node = of_find_compatible_node(NULL, NULL,
			mt6833_spm_wakesrc_irqs[i].name);
		if (!node) {
		...
		mt6833_spm_wakesrc_irqs[i].irq_no =
			irq_of_parse_and_map(node,
				mt6833_spm_wakesrc_irqs[i].order);

		if (!mt6833_spm_wakesrc_irqs[i].irq_no) {
...
struct mt6833_logger_timer {
	struct mt_lpm_timer tm;
	unsigned int fired;
};
struct mt6833_logger_fired_info {
	unsigned int fired;
	int state_index;
};

static struct mt6833_logger_timer mt6833_log_timer;
static struct mt6833_logger_fired_info mt6833_logger_fired;

int mt6833_get_wakeup_status(struct mt6833_log_helper *help)
{
	if (!help->wakesrc || !mt6833_spm_base)
		return -EINVAL;
...
static void mt6833_save_sleep_info(void)
{
...
static void mt6833_suspend_show_detailed_wakeup_reason
	(struct mt6833_spm_wake_status *wakesta)
{
	int i;
	unsigned int irq_no;
	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		if (mt6833_spm_wakesrc_irqs[i].name == NULL ||
			!mt6833_spm_wakesrc_irqs[i].irq_no)
			continue;
		if (mt6833_spm_wakesrc_irqs[i].wakesrc & wakesta->r12) {
			irq_no = mt6833_spm_wakesrc_irqs[i].irq_no;
...
{
#define MT6833_DBG_SMC(_id, _act, _rc, _param) ({\
	...
	for (i = 1 ; i < PLAT_SPM_COND_MAX ; i++) {
		blkcg = MT6833_DBG_SMC(MT_SPM_DBG_SMC_UID_BLOCK_DETAIL, MT_LPM_SMC_ACT_GET, 0, i);
		if (blkcg != 0)
			printk_deferred("suspend warning: CG: %6s = 0x%08lx\n"
				, mt6833_spm_cond_cg_str[i], blkcg);

	}
}

static void mt6833_suspend_spm_rsc_req_check
	(struct mt6833_spm_wake_status *wakesta)
{
...
static int mt6833_show_message(struct mt6833_spm_wake_status *wakesrc, int type,
...
#undef LOG_BUF_SIZE
	...
	if (wakesrc->is_abort != 0) {
		...
		for (i = 1; i < 32; i++) {
			if (wakesrc->r12 & (1U << i)) {
				if (IS_LOGBUF(buf, mt6833_wakesrc_str[i]))
					strncat(buf, mt6833_wakesrc_str[i],
						strlen(mt6833_wakesrc_str[i]));
	...
	if (type == MT_LPM_ISSUER_SUSPEND) {
		printk_deferred("[name:spm&][SPM] %s", log_buf);
		mt6833_suspend_show_detailed_wakeup_reason(wakesrc);
		mt6833_suspend_spm_rsc_req_check(wakesrc);
...
int mt6833_issuer_func(int type, const char *prefix, void *data)
{
	mt6833_get_wakeup_status(&mt6833_logger_help);
	return mt6833_show_message(mt6833_logger_help.wakesrc,
					type, prefix, data);
}

struct mtk_lpm_issuer mt6833_issuer = {
	.log = mt6833_issuer_func,
};

static int mt6833_idle_save_sleep_info_nb_func(struct notifier_block *nb,
			unsigned long action, void *data)
{
	struct mtk_lpm_nb_data *nb_data = (struct mtk_lpm_nb_data *)data;

	if (nb_data && (action == MTK_LPM_NB_BEFORE_REFLECT))
		mt6833_save_sleep_info();

	return NOTIFY_OK;
}

struct notifier_block mt6833_idle_save_sleep_info_nb = {
	.notifier_call = mt6833_idle_save_sleep_info_nb_func,
};

static void mt6833_suspend_save_sleep_info_func(void)
{
	mt6833_save_sleep_info();
}

static struct syscore_ops mt6833_suspend_save_sleep_info_syscore_ops = {
	.resume = mt6833_suspend_save_sleep_info_func,
};
static int mt6833_log_timer_func(unsigned long long dur, void *priv)
{
	struct mt6833_logger_timer *timer =
			(struct mt6833_logger_timer *)priv;
	...
	if (timer->fired != info->fired) {
		 ...
		 */
		if (mt6833_logger_help.prev == mt6833_logger_help.cur)
			mt6833_get_wakeup_status(&mt6833_logger_help);
		mt6833_show_message(mt6833_logger_help.wakesrc,
					MT_LPM_ISSUER_CPUIDLE,
					"MCUSYSOFF", NULL);
		mt6833_logger_help.prev = mt6833_logger_help.cur;
	} else
...
struct notifier_block mt6833_logger_nb = {
	.notifier_call = mt6833_logger_nb_func,
};

static ssize_t mt6833_logger_debugfs_read(char *ToUserBuf,
					size_t sz, void *priv)
{
	char *p = ToUserBuf;
	int len;

	if (priv == ((void *)&mt6833_log_timer)) {
...
static ssize_t mt6833_logger_debugfs_write(char *FromUserBuf,
				   size_t sz, void *priv)
{
	if (priv == ((void *)&mt6833_log_timer)) {
		unsigned int val = 0;
		...
		if (!kstrtouint(FromUserBuf, 10, &val)) {
			if (val == 0)
				mtk_lpm_timer_stop(&mt6833_log_timer.tm);
			else
...
#define MT6833_LOGGER_NODE_INIT(_n, _priv) ({\
	_n.op.fs_read = mt6833_logger_debugfs_read;\
	_n.op.fs_write = mt6833_logger_debugfs_write;\
	_n.op.priv = _priv; })\
...
int mt6833_logger_timer_debugfs_init(void)
{
	mtk_lpm_sysfs_sub_entry_add("logger", 0644,
				NULL, &mt6833_log_tm_node);

	MT6833_LOGGER_NODE_INIT(mt6833_log_tm_interval,
				&mt6833_log_timer);
...
int __init mt6833_logger_init(void)
{
	struct device_node *node = NULL;
	...
	if (node) {
		mt6833_spm_base = of_iomap(node, 0);
		of_node_put(node);
	}

	if (mt6833_spm_base)
		mtk_lp_issuer_register(&mt6833_issuer);
	else
		pr_info("[name:mtk_lpm][P] - Don't register the issue by error! (%s:%d)\n",
			__func__, __LINE__);


	mt6833_get_spm_wakesrc_irq();
	...
	if (drv) {
		...
		for (idx = 0; idx < drv->state_count; ++idx) {
			if (!strcmp(MT6833_LOG_MONITOR_STATE_NAME,
					drv->states[idx].name)) {
				mt6833_logger_fired.state_index = idx;
				break;
			}
		}
	}

	mtk_lpm_notifier_register(&mt6833_logger_nb);
	mtk_lpm_notifier_register(&mt6833_idle_save_sleep_info_nb);

	mt6833_log_timer.tm.timeout = mt6833_log_timer_func;
	mt6833_log_timer.tm.priv = &mt6833_log_timer;
	mtk_lpm_timer_init(&mt6833_log_timer.tm, MTK_LPM_TIMER_REPEAT);
	mtk_lpm_timer_interval_update(&mt6833_log_timer.tm,
					MT6833_LOG_DEFAULT_MS);
	mtk_lpm_timer_start(&mt6833_log_timer.tm);

	register_syscore_ops(&mt6833_suspend_save_sleep_info_syscore_ops);

	return 0;
}
late_initcall_sync(mt6833_logger_init);
