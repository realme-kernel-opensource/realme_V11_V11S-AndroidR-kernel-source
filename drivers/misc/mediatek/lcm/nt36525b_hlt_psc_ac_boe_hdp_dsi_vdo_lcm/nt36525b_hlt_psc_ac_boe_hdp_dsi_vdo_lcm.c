...
** OPLUS_FEATURE_LCD
** File: - nt36525b_psc_ac_hlt_hdp_dsi_vdo_lcm.c
...
static void lcm_set_cabc_cmdq(void *handle, unsigned int level)
{
	...
	if (level == 0) {
		push_table(handle, set_cabc_off,
			sizeof(set_cabc_off) / sizeof(struct LCM_setting_table), 1);

	} else if (level == 1) {
		push_table(handle, set_cabc_ui,
			sizeof(set_cabc_ui) / sizeof(struct LCM_setting_table), 1);

	} else if (level == 2) {
		push_table(handle, set_cabc_still,
			sizeof(set_cabc_still) / sizeof(struct LCM_setting_table), 1);

	} else if (level == 3) {
		push_table(handle, set_cabc_move,
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	push_table_cust(handle, bl_level,
		sizeof(bl_level) / sizeof(struct LCM_setting_table_V3), 0);
	/*//dump_stack();*/
}
...
struct LCM_DRIVER nt36525b_hlt_psc_ac_boe_hdp_dsi_vdo_lcm_drv = {
	.name = "nt36525b_hlt_psc_ac_boe_vdo",
