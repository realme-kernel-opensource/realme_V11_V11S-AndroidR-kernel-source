...
/* ramless fake aod doesn't need set bl in aod screen */
#if FALSE
...
#ifndef BUILD_LK
	register_device_proc("lcd", "S644VA04_MTK03", "samsung1024 cmd mode");
#endif
}
...
static void lcm_aod(int enter)
{
	if (enter == 1) {
		...
		if (aod_light_brightness_mode == 0) {
			lcm_aod_in_setting[10].para_list[0] = 0x22;
		} else {
			lcm_aod_in_setting[10].para_list[0] = 0x23;
		}
		*/
...
static void lcm_aod_display_on(void *handle)
{
	...
	LCD_DEBUG("[soso] lcm aod fake on normal \n");
}
...
struct LCM_DRIVER oppo19151_samsung_ams644va04_1080p_dsi_cmd_lcm_drv =
{
	.name = "oppo19151_samsung_ams644va04_1080p_dsi_cmd",
