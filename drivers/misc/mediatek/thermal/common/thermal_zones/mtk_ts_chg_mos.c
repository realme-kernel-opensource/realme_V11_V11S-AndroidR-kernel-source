...
static int pre_temp_idx = 50;  //start from 30 'c
...
static int mtk_chg_mos_get_temp(struct thermal_zone_device *thermal, int *t)
{
	...
	real_temp = CHG_MOS_Temperature_Table[pre_temp_idx].NTC_Temp * 1000;

	if (temp_uv < CHG_MOS_Temperature_Table[pre_temp_idx].TemperatureR) {
