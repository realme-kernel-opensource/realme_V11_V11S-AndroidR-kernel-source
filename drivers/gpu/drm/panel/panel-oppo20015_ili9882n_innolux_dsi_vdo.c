...
/* shiyaqiang@RM.MM.LCD.Driver 20180621 add for keep cabc mode after resume*/
	...
	if ((level > oplus_max_normal_brightness) && (is_highlight_flag == false)) {
                cabc_switch(dsi,cb,handle,0);
                is_highlight_flag = true;
        } else if ((level <= oplus_max_normal_brightness) && is_highlight_flag == true) {
                cabc_switch(dsi,cb,handle,cabc_lastlevel);
                is_highlight_flag = false;
        }
	return 0;
}
		...
		void *handle, unsigned int cabc_mode)
{
	...
	pr_err("%s cabc = %d\n", __func__, cabc_mode);
	if (last_brightness > oplus_max_normal_brightness)
		bl_tb0[1] = 0;
	else
...
static int lcm_probe(struct mipi_dsi_device *dsi)
{
	...
	if (is_project(20651) || is_project(20652) || is_project(21637) || is_project(21638))
		disp_aal_set_dre_en(1);
	if (is_project(20651) || is_project(20652) || is_project(21637) || is_project(21638)) {
		register_device_proc("lcd","ili9882n_innolux_carr","INNOLUX");
	} else {
		register_device_proc("lcd","ili9882n_innolux","INNOLUX");
	}
	pr_info("%s-\n", __func__);
...
static const struct of_device_id jdi_of_match[] = {
	{
		.compatible = "innolux,ili9882n,dphy,vdo",
	},
	{} };
...
static struct mipi_dsi_driver jdi_driver = {
	...
	.remove = lcm_remove,
	.driver = {
			.name = "oppo20015_ili9882n_innolux_vdo_lcm_drv",
			.owner = THIS_MODULE,
