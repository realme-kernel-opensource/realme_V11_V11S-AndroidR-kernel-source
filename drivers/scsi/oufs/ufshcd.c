...
#ifdef OPLUS_FEATURE_STORAGE_TOOL
	UFS_FIX(UFS_VENDOR_SAMSUNG, "KM8V8001JM-B813",
		UFS_DEVICE_QUIRK_FFU_RESET),
	UFS_FIX(UFS_VENDOR_SAMSUNG, "KM8V7001JM-B810",
		UFS_DEVICE_QUIRK_FFU_RESET),
#endif
...
/* hexiaosen@BSP.Storage.UFS 2020-08-13 add for ufs reset after ffu write buffer
   ...
   the doorbells of transfer requests and task management requests to be cleared. */
	if (hba->dev_quirks & UFS_DEVICE_QUIRK_FFU_RESET) {
		if ((UFS_FFU_DOWNLOAD_MODE == cmd->req.cmd[1]) &&
			(WRITE_BUFFER == cmd->req.cmd[0])) {
			while(retry--) {
...
/* hexiaosen@BSP.Storage.UFS 2020-08-13 add for ufs reset after ffu write buffer */
			if (hba->dev_quirks & UFS_DEVICE_QUIRK_FFU_RESET) {
				if ((UFS_FFU_DOWNLOAD_MODE == scmd[1]) &&
					(WRITE_BUFFER == scmd[0])) {
