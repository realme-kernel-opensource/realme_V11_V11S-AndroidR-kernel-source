...
static int32_t nvt_check_esd_recovery_count(struct chip_data_nt36672c *chip_info)
{
	unsigned int timer = jiffies_to_msecs(jiffies - chip_info->recovery_timer);

	if (timer < NVT_TOUCH_ESD_CHECK_PERIOD) {
		chip_info->recovery_period_cnt++;
		TPD_INFO("timer = %d, recovery_period_cnt = %d/%d\n",
				timer, chip_info->recovery_period_cnt, NVT_TOUCH_ESD_CHECK_COUNT_MAX);
	} else {
		chip_info->recovery_period_cnt = 0;
	}
	
	chip_info->recovery_timer = jiffies;
	
	if (chip_info->recovery_period_cnt >= NVT_TOUCH_ESD_CHECK_COUNT_MAX) {
		return 1;
	} else {
		return 0;
	}
...
#if NVT_TOUCH_ESD_DISP_RECOVERY
        if (nvt_check_esd_recovery_count(chip_info)) {
            TPD_INFO("set display off to trigger display esd recovery.\n");
            nvt_f2c_disp_off(chip_info);
            return -1;
        }
