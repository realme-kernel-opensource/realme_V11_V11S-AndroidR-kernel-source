                                        ...
                                        bool force)
{
    ...
    chip_info->actual_tp_mode = P5_X_FW_AP_MODE;
    ilits->oppo_fw_update = true;
    ret = ili_fw_upgrade_handler();
    ilits->oppo_fw_update = false;
    mutex_unlock(&chip_info->touch_mutex);
                                  ...
                                  u8 read_type)
{
    ...
    atomic_set(&ilits->cmd_int_check, ENABLE);
    enable_irq(ilits->irq_num);//because oppo disable
    ...
    if (i < 10) {
    	...
    	seq_printf(s, "Y Data:");
		for (i = 2 * xch * ych + 35; i < 2 * xch * ych + 2 * ych + 35; i += 2) {
			s16 temp;
		...
		seq_printf(s, "X Data:");
		for (i = 2 * xch * ych + 2 * ych + 35; i < 2 * xch * ych + 2 * ych + 2 * xch + 35; i += 2) {
			s16 temp;
			temp = (s16)((buf[i] << 8) + buf[i + 1]);
			seq_printf(s, "%5d,", temp);
		}
		...
		for (i = 2 * xch * ych + 2 * ych + 2 * xch + 35; i < ilits->tp_data_len - 1; i += 2) {
			s16 temp;
			temp = (s16)((buf[i] << 8) + buf[i + 1]);
			seq_printf(s, "%5d,", temp);
		}
...
static int ilitek_create_proc_for_oppo(struct touchpanel_data *ts)
{
    int ret = 0;
...
void  tp_driver_exit_ili_9882n(void)
{
