...
enum Tfa98xx_Error tfa98xx_set_mtp(struct tfa_device *tfa,
			      uint16_t value,
			      uint16_t mask);
enum Tfa98xx_Error tfa98xx_get_mtp(struct tfa_device *tfa,
			      uint16_t *value);
 ...
 */
enum Tfa98xx_Error
tfa98xx_dsp_get_sw_feature_bits(struct tfa_device *tfa,
			      int features[2]);
enum Tfa98xx_Error
tfa98xx_dsp_get_hw_feature_bits(struct tfa_device *tfa,
			      int *features);
 ...
 */
enum Tfa98xx_Error tfa98xx_dsp_write_preset_v6(struct tfa_device *tfa,
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_id(struct tfa_device *tfa,
			      int length,
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_write(struct tfa_device *tfa,
			      int length,
			      const char *buffer);
 ...
 */
enum Tfa98xx_Error tfa_dsp_msg_v6_read(struct tfa_device *tfa,
			      int length,
			      unsigned char *bytes);
...
int tfa_set_bf_v6_volatile(struct tfa_device *tfa,
			      const uint16_t bf,
			      const uint16_t value);
...
#define TFA_GET_BF_VALUE(tfa, fieldname, reg_value) \
	tfa_get_bf_v6_value(TFA_FAM(tfa, fieldname), reg_value)
...
#define TFA98XX_API_WAIT_TRIES 3000 // defined in API
