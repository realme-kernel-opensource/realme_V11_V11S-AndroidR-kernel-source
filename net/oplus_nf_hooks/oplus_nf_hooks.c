...
/* send to user space */
static int oplus_nf_hooks_send_to_user(int msg_type, char *payload,
				       int payload_len)
{
	int ret = 0;
	...
	if (oplus_nf_hooks_pid) {
		ret = netlink_unicast(oplus_nf_hooks_sock, skbuff, oplus_nf_hooks_pid,
				      MSG_DONTWAIT);

	} else {
		printk(KERN_ERR
		       "oplus_nf_hooks_netlink: can not unicast skbuff, oplus_nf_hooks_pid=0\n");
		kfree_skb(skbuff);
	}
	...
	if ((iph = ip_hdr(skb)) != NULL && iph->protocol == IPPROTO_TCP) {
		if (is_wechat_skb(ct, skb)) {
			...
			for (i = 0; i < wechat_param_count; i++) {
				if (tot_len >= wechat_infos[i].min_tot_len
						&& tot_len <= wechat_infos[i].max_tot_len) {
		...
		const struct nf_hook_state *state)
{
	...
	if (skb->protocol == htons(ETH_P_IPV6) && (ipv6h = ipv6_hdr(skb)) != NULL
		...
		if (is_wechat_skb(ct, skb)) {
			...
			header_len = ihl + tcph->doff *
				     4;  /* total length of ipv6 header and tcp header */
...
static int oplus_nf_hooks_set_wechat_param(struct nlmsghdr *nlh)
{
	...
	if (nlh->nlmsg_len == NLMSG_HDRLEN + 2 * sizeof(u32) + wechat_param_count *
			sizeof(
				struct wechat_pattern_info)) {
...
static int oplus_nf_hooks_netlink_init(void)
{
	...
	oplus_nf_hooks_sock = netlink_kernel_create(&init_net, NETLINK_OPLUS_NF_HOOKS,
			      &cfg);
...
static int __init oplus_nf_hooks_init(void)
{
	...
	ret |= nf_register_net_hooks(&init_net, oplus_nf_hooks_ops,
...
static void __exit oplus_nf_hooks_fini(void)
{
	...
	nf_unregister_net_hooks(&init_net, oplus_nf_hooks_ops,
				ARRAY_SIZE(oplus_nf_hooks_ops));
}
