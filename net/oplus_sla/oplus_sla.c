...
**Junyuan.Huang@CONNECTIVITY.WIFI.INTERNET 2018/04/03     1.0      OPLUS_FEATURE_WIFI_SLA
 OPPO Java File Static Checking Skip
 *******************************************************************************/
...
struct oplus_smart_bw_rom_update_info {
	int SELECT_BW_WHEN_CONNECT_FEATRUE_ENABLE;
	int DYNAMIC_SW_BW_FEATRUE_ENABLE;
	int DBG;

	int SCAN_RESULT_EXPIRE;
	int SAVE_CONNECT_LOG_THRESHOLD;
	int CLEAN_AP_NUMBER;
	int GOOD_RSSI;
	int CLEAN_CH_IDLE_SLOT_RATIO;
	int CLEAN_BAND_IDLE_SLOT_RATIO;

	int LQ_CHECK_NUM;
	int LQ_MIN_ACTIVE_NEED_NUM;
	int LQ_CHECK_EXPIRE;
	int LQ_AP_40M_IOT_L2_REACH_PACKET_COUNT;
	int LQ_AP_40M_IOT_L3_REACH_PACKET_COUNT;
	int LQ_40M_BAD_RATE;
	int LQ_PER_HIGH;
	int LQ_IDLE_SLOT_RATIO_ACCEPT;
	int LQ_L3_SPEED_LOW;
};
...
*/
extern void (*mark_streams_for_iptables_reject)(struct sk_buff *skb,
...
static int oplus_smart_bw_set_params(struct nlmsghdr *nlh)
{
	...
	if (count == sizeof(struct oplus_smart_bw_rom_update_info)) {
		...
		smart_bw_rom_update_info.SELECT_BW_WHEN_CONNECT_FEATRUE_ENABLE = params[0];
		smart_bw_rom_update_info.DYNAMIC_SW_BW_FEATRUE_ENABLE = params[1];
		smart_bw_rom_update_info.DBG = params[2];

		smart_bw_rom_update_info.SCAN_RESULT_EXPIRE = params[3];
		smart_bw_rom_update_info.SAVE_CONNECT_LOG_THRESHOLD = params[4];
		smart_bw_rom_update_info.CLEAN_AP_NUMBER = params[5];
		smart_bw_rom_update_info.GOOD_RSSI = params[6];
		smart_bw_rom_update_info.CLEAN_CH_IDLE_SLOT_RATIO = params[7];
		smart_bw_rom_update_info.CLEAN_BAND_IDLE_SLOT_RATIO = params[8];

		smart_bw_rom_update_info.LQ_CHECK_NUM = params[9];
		smart_bw_rom_update_info.LQ_MIN_ACTIVE_NEED_NUM = params[10];
		smart_bw_rom_update_info.LQ_CHECK_EXPIRE = params[11];
		smart_bw_rom_update_info.LQ_AP_40M_IOT_L2_REACH_PACKET_COUNT = params[12];
		smart_bw_rom_update_info.LQ_AP_40M_IOT_L3_REACH_PACKET_COUNT = params[13];
		smart_bw_rom_update_info.LQ_40M_BAD_RATE = params[14];
		smart_bw_rom_update_info.LQ_PER_HIGH = params[15];
		smart_bw_rom_update_info.LQ_IDLE_SLOT_RATIO_ACCEPT = params[16];
		smart_bw_rom_update_info.LQ_L3_SPEED_LOW = params[17];

		if (smart_bw_rom_update_info.DBG && oplus_sla_debug) {
				printk("opus_smart_bw_set_params:set params count=%d params[0] = %d,params[1] = %d,params[2] = %d,params[3] = %d,"
						...
						params[9],params[10],params[11],params[12],params[13],params[14],params[15],params[16],params[17]);
        }
...
{
	if (len < sizeof(struct oplus_smart_bw_rom_update_info) / sizeof(int)
			|| !get_smartbw_romupdate) {
		if (1) {
			printk("%s target payload len not match or hasn't got ROM update from FWK !\n",
			       __func__);
		}
	...
	memcpy(payload, &smart_bw_rom_update_info,
	       sizeof(struct oplus_smart_bw_rom_update_info));
...
/* Get L3 RTT Congestion Current Speed, return true: success */
bool get_L3_LQ_status(int payload[], int len)
{
	if (len < 5) {
	        return false;
	}

	if (oplus_sla_debug) printk("%s enter!\n", __func__);

	if (oplus_sla_info[MAIN_WLAN].if_up || oplus_sla_info[MAIN_WLAN].need_up ||
	    oplus_sla_info[CELL_INDEX].if_up || oplus_sla_info[CELL_INDEX].need_up) {
	    payload[0] = oplus_sla_info[MAIN_WLAN].avg_rtt;
	    payload[1] = oplus_sla_info[MAIN_WLAN].current_speed;
	    payload[2] = oplus_sla_info[MAIN_WLAN].max_speed;
	    payload[3] = oplus_sla_info[MAIN_WLAN].congestion_flag;

	    payload[4] = oplus_sla_enable;

	    if (oplus_sla_debug) {
	        printk("oplus_sla:get_rtt_speed wlan:max_speed=%d, avg_rtt=%d  "
	               "cell:max_speed=%d, avg_rtt=%d, oplus_sla_enable=%d\n",
	               payload[0], payload[1], payload[2], payload[3], oplus_sla_enable);
	    }
	}
	if (oplus_sla_debug) printk("%s exit!\n", __func__);
	return true;
}
EXPORT_SYMBOL(get_L3_LQ_status);
		...
		if (oplus_sla_info[MAIN_WLAN].download_speed >= MAX_WLAN_SPEED) {
			...
			choose_mark = MAIN_WLAN_MARK;
				...
				(oplus_sla_info[MAIN_WLAN].download_speed <= VIDEO_SPEED &&
				 ((init_weight_delay_count > 0
				   && oplus_sla_info[MAIN_WLAN].dl_little_speed < DOWNLOAD_SPEED) ||
				  (oplus_sla_info[SECOND_WLAN].download_speed > VIDEO_SPEED
				   && oplus_sla_info[SECOND_WLAN].sla_avg_rtt <= 150)))) {
		...
		if (NULL != th && th->syn &&
			...
			*/
			...
			if (tmp_mark != MAIN_WLAN_MARK) {
				...
				if (dev && !memcmp(dev->name, oplus_sla_info[MAIN_WLAN].dev_name,
						   strlen(dev->name))) {
					if (oplus_sla_debug) {
...
static void game_app_switch_network(u32 game_type)
{
	...
	if (cell_quality_good &&
		...
		if (!game_uid[index].switch_time ||
			...
			oplus_sla_send_to_user(SLA_SWITCH_GAME_NETWORK, (char *)game_bp_info,
					       sizeof(game_bp_info));
...
static bool is_game_rtt_skb(struct nf_conn *ct, struct sk_buff *skb, bool isTx)
{
	...
	if (gameType <= 0 || gameType >= GAME_NUM
			|| game_params[gameType].game_index == 0 ||
	...
	if (isTx) {
		...
		if (udph->len >= (tx_offset + tx_len) &&
				memcmp(payload + tx_offset, game_params[gameType].tx_fixed_value,
				       tx_len) == 0) {
			if (oplus_sla_debug) {
		...
		if (udph->len >= (rx_offset + rx_len) &&
				memcmp(payload + rx_offset, game_params[gameType].rx_fixed_value,
				       rx_len) == 0) {
			if (oplus_sla_debug) {
		...
		if (is_game_rtt_skb(ct, skb, true)) {
			...
			if (ct->oplus_game_timestamp && time_now - ct->oplus_game_timestamp > 200) {
				...
				if (game_rtt_show_toast) {
					...
					oplus_sla_send_to_user(SLA_NOTIFY_GAME_RTT, (char *)&game_rtt,
							       sizeof(game_rtt));
				}
	...
	if ((iph = ip_hdr(skb)) != NULL && iph->protocol == IPPROTO_UDP) {
		...
		if (game_type == GAME_WZRY) {
			if (udph->len >= (wzry_offset + wzry_len) &&
					memcmp(payload + wzry_offset, wzry_fixed_value,
					       wzry_len) == 0) {	/* for cjzc voice pkt */
			...
			if (udph->len >= (cjzc_offset + cjzc_len) &&
					memcmp(payload + cjzc_offset, cjzc_fixed_value,
					       cjzc_len) == 0) {	/* for cjzc voice pkt */
				/* printk("oplus_sla_game_rx_voice:this is voice skb!\n"); */
				ct->oplus_game_detect_status |= GAME_VOICE_STREAM;
				return true;
...
/* add for android Q statictis tcp tx and tx */
static void statistics_wlan_tcp_tx_rx(const struct nf_hook_state *state,
				      struct sk_buff *skb)
{
	struct iphdr *iph;
	struct nf_conn *ct = NULL;
		...
		if (NULL == ct || nf_ct_is_untracked(ct) || (0 == ct->mark)) {
			...
			if (dev && !memcmp(dev->name, oplus_sla_info[MAIN_WLAN].dev_name,
				...
				if (NF_INET_LOCAL_OUT == hook) {
					...
					&& !memcmp(dev->name, oplus_sla_info[SECOND_WLAN].dev_name,
						   strlen(dev->name))) {
				if (NF_INET_LOCAL_OUT == hook) {
					wlan1_tcp_tx++;
		...
		if (ct->oplus_game_timestamp && time_now > ct->oplus_game_timestamp) {
			...
			if (oplus_sla_debug) {
				...
				printk("oplus_sla_game_rtt: game_rtt=%u, srtt=%u\n", game_rtt,
				       game_uid[ct->oplus_app_type].rtt >> shift);
			}
		}
	}
...
/*
  ...
  */
static void get_content_lenght(struct nf_conn *ct, struct sk_buff *skb,
				...
				if ((datalen - header_len) >= 64) { /* ip->len > tcphdrlen */
					...
					if (oplus_speed_info[index].ms_speed_flag) {
						...
						if (tmp_time > 500000000) {
							...
							tmp_time = oplus_speed_info[index].last_time -
								   oplus_speed_info[index].first_time;
							tmp_speed = (1000 * 1000 * oplus_speed_info[index].rx_bytes) /
								    (tmp_time); /* kB/s */
...
*so just mark the stream(ct) with mark IPTABLE_REJECT_MARK when this happens
*/
static void sla_mark_streams_for_iptables_reject(struct sk_buff *skb,
		enum ipt_reject_with reject_type)
{
	struct nf_conn *ct = NULL;
	enum ip_conntrack_info ctinfo;
		...
		if (oplus_sla_info[i].if_up || oplus_sla_info[i].need_up) {
			...
			if (oplus_sla_debug /*&& net_ratelimit()*/) {
				printk("oplus_sla: dev_name = %s,if_up = %d,max_speed = %d,"
				       ...
				       oplus_sla_info[i].download_speed, oplus_sla_info[i].dual_wifi_download,
				       oplus_sla_info[i].dl_little_speed);
			}
...
calc_weight_finish:
	...
	if (oplus_sla_debug) {
		printk("oplus_sla_weight: work_mode = %d,weight[%d] = %d,weight[%d] = %d\n",
		       sla_work_mode, index1, oplus_sla_info[index1].weight, index2,
		       oplus_sla_info[index2].weight);
	}
...
finish_calc:
	...
	if (oplus_sla_debug) {
		printk("oplus_sla_weight: work_mode = %d,weight[0] = %d,weight[1] = %d,weight[2] = %d\n",
		       sla_work_mode, oplus_sla_info[0].weight, oplus_sla_info[1].weight,
...
static int wlan_can_enable_second_wifi(int index)
{
	...
	if (oplus_sla_info[index].if_up ||
		...
		if (oplus_sla_info[index].dual_wifi_download >= DOWNLOAD_FLAG &&
				oplus_sla_info[index].download_speed <
				rom_update_info.dual_wlan_download_speed) {
...
static bool dual_wlan_enable_cell(void)
{
	...
	if (wlan_can_enable_cell(MAIN_WLAN) &&
		...
		sum_speed = oplus_sla_info[MAIN_WLAN].max_speed +
			    oplus_sla_info[SECOND_WLAN].max_speed;
	...
	for (index = 0; index < IFACE_NUM; index++) {
		...
		if (oplus_sla_info[index].if_up || oplus_sla_info[index].need_up) {
			...
			if (oplus_sla_info[index].rtt_index >= RTT_NUM) {
				...
				oplus_sla_info[index].avg_rtt = (7 * oplus_sla_info[index].avg_rtt + avg_rtt) /
								8;
			...
			if (oplus_sla_debug) {
				printk("oplus_sla_rtt: index = %d,wlan_rtt = %d,"
				       ...
				       oplus_sla_enable, oplus_sla_def_net, avg_rtt, sla_detect_mode, sla_work_mode,
				       game_cell_to_wifi);
			}
		}
	}
...
static void game_rx_update(void)
{
	if (!oplus_sla_vpn_connected &&
		...
		cur_info->udp_rx_packet[cur_info->window_index % UDP_RX_WIN_SIZE] =
			delta_udp_pkts;
		cur_info->tcp_rx_byte[cur_info->window_index % TCP_RX_WIN_SIZE] =
			delta_tcp_bytes;
		cur_info->window_index++;

		/* deal with the control logic */
		if (udp_rx_show_toast) {
			oplus_sla_send_to_user(SLA_NOTIFY_GAME_RX_PKT, (char *)&delta_udp_pkts,
					       sizeof(delta_udp_pkts));
		}
		...
		if (tcp_rx_large_count == TCP_RX_WIN_SIZE) {
			...
			if (inGame && cur_info->game_in_front) {
				if (lowTrafficCount >= cur_info->rx_bad_thr &&
						game_index > 0
						&& game_start_state[game_index]) {/* wzry has low traffic when game ends */
					game_rx_bad = true;
...
static int oplus_sla_set_game_rtt_params(struct nlmsghdr *nlh)
{
	...
	if (index > 0 && index < GAME_NUM) {
		...
		printk("oplus_sla_netlink:set game params index=%d tx_offset=%d tx_len=%d tx_fix=",
		       game_params[index].game_index, game_params[index].tx_offset,
		       game_params[index].tx_len);

		for (i = 0; i < game_params[index].tx_len; i++) {
			printk("%02x", game_params[index].tx_fixed_value[i]);
		}

		printk(" rx_offset=%d rx_len=%d rx_fix=", game_params[index].rx_offset,
		       game_params[index].rx_len);
