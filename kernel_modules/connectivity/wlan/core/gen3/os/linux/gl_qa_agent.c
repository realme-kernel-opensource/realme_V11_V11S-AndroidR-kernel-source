			   ...
			   IN union iwreq_data *prIwReqData, INT_32 i4Length, INT_32 i4Status)
{
	...
	if (copy_to_user(prIwReqData->data.pointer, (UCHAR *) (HqaCmdFrame), prIwReqData->data.length)) {
		DBGLOG(RFTEST, ERROR, " QA_AGENT copy_to_user() fail in %s\n", __func__);
		return (-EFAULT);
	}
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA command(0x%04x)[Magic number(0x%08x)] is done,i4Status(%d)\n",
	       ntohs(HqaCmdFrame->Id), ntohl(HqaCmdFrame->MagicNo), i4Status);
...
static INT_32 HQA_StartTx(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_StartTx TxCount = %u, TxLength = %u\n", TxCount, TxLength);
...
static INT_32 HQA_SetTxIPG(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetTxIPG u4Aifs : %u\n", u4Aifs);
						 ...
						 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_FWPacketCMD_ClockSwitchDisable\n");
...
static INT_32 HQA_SetChannel(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetChannel Channel = %d\n", i4SetChan);
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetPerPktBW u4Perpkt_bw = %u\n", u4Perpkt_bw);
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetPrimaryBW u4Pri_sel : %d\n", u4Pri_sel);
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetFreqOffset u4FreqOffset : %d\n", u4FreqOffset);
						      ...
						      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRxHighLowTemperatureCompensation\n");
				       ...
				       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT u4Item : 0x%08x, u4Band_idx : %u\n", u4Item, u4Band_idx);
				 ...
				 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRXVectorIdx band_idx : %u, Group_1 : %u, Group_2 : %u\n",
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT u4band_idx : %d, u4FAGC_Path : %d\n", u4band_idx, u4FAGC_Path);
...
/*	INT_32 i4Ret = 0; */
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MacBbpRegWrite Offset = 0x%08x, Value = 0x%08x\n", u4Offset, u4Value);
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_RfRegBulkRead WfSel  = %u, Offset = 0x%08x, Length = %u\n",
				 ...
				 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT WfSel  = %u, Offset = 0x%08x, Length = %u, Value  = 0x%08x\n",
				 ...
				 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	UINT_16  u4Value = 0;
	UINT_16 Buffer[EFUSE_BLOCK_SIZE>>1];
	P_GLUE_INFO_T prGlueInfo = NULL;
	WLAN_STATUS rStatus = WLAN_STATUS_SUCCESS;
	...
	return 0;
#if 0 /* currently not used when fix coverity issue */
	memcpy(&Offset, HqaCmdFrame->Data + 2 * 0, 2);
	Offset = ntohs(Offset);
	memcpy(&Len, HqaCmdFrame->Data + 2 * 1, 2);
	Len = ntohs(Len);

	DBGLOG(INIT, INFO, " QA_AGENT HQA_ReadBulkEEPROM Offset : %u, Len : %u\n", Offset, Len);

#if  (CFG_EEPROM_PAGE_ACCESS == 1)
	rAccessEfuseInfo.u4Address = (Offset / EFUSE_BLOCK_SIZE) * EFUSE_BLOCK_SIZE;

	DBGLOG(INIT, INFO, " QA_AGENT HQA_ReadBulkEEPROM Address : %d\n", rAccessEfuseInfo.u4Address);
	...
	if (rStatus == WLAN_STATUS_FAILURE)
		DBGLOG(INIT, INFO, " QA_AGENT HQA_ReadBulkEEPROM kal fail\n");



	for (u4Loop = 0; u4Loop < (EFUSE_BLOCK_SIZE>>1); u4Loop++) {

		u4Index = u4Loop % EFUSE_BLOCK_SIZE;
		if (u4Index <= 14)
			u4Value = (prGlueInfo->prAdapter->aucEepromVaule[u4Index])
						| (prGlueInfo->prAdapter->aucEepromVaule[u4Index+1] << 8);


		Buffer[u4Loop] = u4Value;
		DBGLOG(INIT, INFO, " QA_AGENT HQA_ReadBulkEEPROM u4Loop=%d  u4Value=%x\n", u4Loop, u4Value);

	}

	if (Offset + Len <= EEPROM_SIZE)
		kalMemCopy(HqaCmdFrame->Data + 2, (UINT_8 *)Buffer, EFUSE_BLOCK_SIZE);
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(INIT, INFO, " QA_AGENT HQA_WriteBulkEEPROM Offset : %x, Len : %u\n", Offset, Len);
	...
	for (u4Loop = 0; u4Loop < (EFUSE_BLOCK_SIZE>>1); u4Loop++) {
		...
		DBGLOG(INIT, INFO, " QA_AGENT HQA_WriteBulkEEPROM u4Loop=%d  u4Value=%x\n",
	...
	kalMemSet(&rAccessEfuseInfoWrite, 0, sizeof(PARAM_CUSTOM_ACCESS_EFUSE_T));
	u4Index = Offset % EFUSE_BLOCK_SIZE;
	DBGLOG(INIT, INFO, " QA_AGENT HQA_WriteBulkEEPROM  Write, u4Index=%x, testBuffer=%x\n",
			u4Index, testBuffer);

	DBGLOG(INIT, INFO, " Buffer[0]=%x, Buffer[0]&0xff=%x\n", Buffer[0], Buffer[0]&0xff);
	DBGLOG(INIT, INFO, " Buffer[0] >> 8 & 0xff=%x\n", Buffer[0] >> 8 & 0xff);


	prGlueInfo->prAdapter->aucEepromVaule[u4Index] = testBuffer;
	prGlueInfo->prAdapter->aucEepromVaule[u4Index+1] = testBuffer >> 8 & 0xff;
	...
	if (Len == 2) {
		...
		if (!g_BufferDownload) {
			...
			if (prGlueInfo->prAdapter->rWifiVar.ucEfuseBufferModeCal == TRUE) { /* structure for MT7668 */

				for (j = 0 ; j < 16 ; j++) {
					/* rSetEfuseBufModeInfo.aBinContent[j].u2Addr = u2InitAddr; */
					/* rSetEfuseBufModeInfo.aBinContent[j].ucValue = uacEEPROMImage[u2InitAddr]; */

					rSetEfuseBufModeInfo_1.aBinContent[j] = uacEEPROMImage[u2InitAddr];
					u2InitAddr += 1;
				}

				rSetEfuseBufModeInfo_1.ucSourceMode = 1;
				rSetEfuseBufModeInfo_1.ucCount = (UINT_8)EFUSE_CONTENT_SIZE_1;
				rStatus = kalIoctl(prGlueInfo,
									wlanoidSetEfusBufferMode,
									&rSetEfuseBufModeInfo_1,
									sizeof(PARAM_CUSTOM_EFUSE_BUFFER_MODE_1_T),
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	rStatus = kalIoctl(prGlueInfo,
				...
				&rEfuseFreeBlock,
				sizeof(PARAM_CUSTOM_EFUSE_FREE_BLOCK_T), FALSE, FALSE, TRUE, &u4BufLen);

	kalMemCopy(HqaCmdFrame->Data + 2, &(prGlueInfo->prAdapter->u4FreeBlockNum), 4);
#endif
...
{
	INT_32 i4Ret = 0;
	UINT_32 u4Channel = 0, u4Band = 0, u4Ch_Band = 0;
/*	UINT_32 u4EfuseAddr = 0, u4Power = 0; */
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_GetTxPower u4Channel : %u, u4Band : %u, u4Ch_Band : %u\n",
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetCfgOnOff Type : %u, Enable : %u, Band : %u\n", Type, Enable, Band);
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (i4Ret == 0) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_GetFreqOffset u4FreqOffset = %d\n", u4FreqOffset);
				   ...
				   IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Band : %d\n", u4Band);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Control : %d\n", u4Control);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4AntMask : %d\n", u4AntMask);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Phymode : %d\n", u4Phymode);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4BW : %d\n", u4BW);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Pri_Ch : %d\n", u4Pri_Ch);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Rate : %d\n", u4Rate);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4Central_Ch : %d\n", u4Central_Ch);	/* ok */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DBDCContinuousTX u4TxfdMode : %d\n", u4TxfdMode);	/* ok */
	...
	if (u4Control) {
		...
		if (u4Phymode == 0) {
			...
			DBGLOG(RFTEST, INFO, " QA_AGENT CCK/OFDM (normal preamble) rate : %d\n", u4Rate);
			...
			DBGLOG(RFTEST, INFO, " QA_AGENT CCK (short preamble) rate : %d\n", u4Rate);
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRXFilterPktLen Band : %d\n", u4Band);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRXFilterPktLen Control : %d\n", u4Control);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRXFilterPktLen RxPktlen : %d\n", u4RxPktlen);
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetRXFilterPktLen rRfATInfo.u4FuncData : 0x%08x\n",
	       rRfATInfo.u4FuncData);
...
static INT_32 HQA_GetTXInfo(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (i4Status == 0) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT u4Txed_band0 packet count = %d\n", u4Txed_band0);
	...
	if (i4Status == 0) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT u4Txed_band1 packet count = %d\n", u4Txed_band1);
...
static INT_32 HQA_SetShortGI(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetShortGI u4ShortGi = %d\n", u4ShortGi);
				     ...
				     IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	u4RxStatSeqNum++;
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.ACIHitLower : 0x%08x\n", g_HqaRxStat.ACIHitLower);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.ACIHitUpper : 0x%08x\n", g_HqaRxStat.ACIHitUpper);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllFCSErr0 : 0x%08x\n", g_HqaRxStat.AllFCSErr0);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllFCSErr1 : 0x%08x\n", g_HqaRxStat.AllFCSErr1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllLengthMismatchCount_B0 : 0x%08x\n",
					g_HqaRxStat.AllLengthMismatchCount_B0);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllLengthMismatchCount_B1 : 0x%08x\n",
					g_HqaRxStat.AllLengthMismatchCount_B1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllMacMdrdy0 : 0x%08x\n", g_HqaRxStat.AllMacMdrdy0);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.AllMacMdrdy1 : 0x%08x\n", g_HqaRxStat.AllMacMdrdy1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_FCS_Err_Band1 : 0x%08x\n", g_HqaRxStat.CCK_FCS_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_PD : 0x%08x\n", g_HqaRxStat.CCK_PD);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_PD_Band1 : 0x%08x\n", g_HqaRxStat.CCK_PD_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_SFD_Err : 0x%08x\n", g_HqaRxStat.CCK_SFD_Err);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_SFD_Err_Band1 : 0x%08x\n", g_HqaRxStat.CCK_SFD_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_SIG_Err : 0x%08x\n", g_HqaRxStat.CCK_SIG_Err);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.CCK_SIG_Err_Band1 : 0x%08x\n", g_HqaRxStat.CCK_SIG_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.DriverRxCount : 0x%08x\n", g_HqaRxStat.DriverRxCount);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.DriverRxCount1 : 0x%08x\n", g_HqaRxStat.DriverRxCount1);
	DBGLOG(RFTEST, TRACE, " FAGCRssiIBR0:0x%08x, FAGCRssiIBR0:0x%08x, FAGCRssiIBR0:0x%08x, FAGCRssiIBR0:0x%08x\n",
	g_HqaRxStat.FAGCRssiIBR0, g_HqaRxStat.FAGCRssiIBR1, g_HqaRxStat.FAGCRssiIBR2, g_HqaRxStat.FAGCRssiIBR3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.FAGCRssiIBR1 : 0x%08x, FAGCRssiIBR2 : 0x%08x, FAGCRssiIBR3 : 0x%08x\n",
		g_HqaRxStat.FAGCRssiIBR1, g_HqaRxStat.FAGCRssiIBR2, g_HqaRxStat.FAGCRssiIBR3);
	DBGLOG(RFTEST, TRACE,
		"FAGCRssiWBR0 : 0x%08x, FAGCRssiWBR1 : 0x%08x, FAGCRssiWBR2 : 0x%08x, FAGCRssiWBR3 : 0x%08x\n",
		g_HqaRxStat.FAGCRssiWBR0, g_HqaRxStat.FAGCRssiWBR1, g_HqaRxStat.FAGCRssiWBR2, g_HqaRxStat.FAGCRssiWBR3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.FCSErr_CCK : 0x%08x\n", g_HqaRxStat.FCSErr_CCK);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.FCSErr_OFDM : 0x%08x\n", g_HqaRxStat.FCSErr_OFDM);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.FreqOffsetFromRX : 0x%08x\n", g_HqaRxStat.FreqOffsetFromRX);
	DBGLOG(RFTEST, TRACE, "IB_RSSI0 : 0x%08x, IB_RSSI1 : 0x%08x\n", g_HqaRxStat.IB_RSSI0, g_HqaRxStat.IB_RSSI1);
	DBGLOG(RFTEST, TRACE, "InstRssiIBR0:0x%08x, InstRssiIBR1:0x%08x, InstRssiIBR2:0x%08x, InstRssiIBR3:0x%08x\n",
	g_HqaRxStat.InstRssiIBR0, g_HqaRxStat.InstRssiIBR1, g_HqaRxStat.InstRssiIBR2, g_HqaRxStat.InstRssiIBR3);

	DBGLOG(RFTEST, TRACE, "InstRssiWBR0:0x%08x, InstRssiWBR1:0x%08x, InstRssiWBR2:0x%08x, InstRssiWBR3:0x%08x\n",
		g_HqaRxStat.InstRssiWBR0, g_HqaRxStat.InstRssiWBR1, g_HqaRxStat.InstRssiWBR2, g_HqaRxStat.InstRssiWBR3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.LengthMismatchCount_B0 : 0x%08x\n", g_HqaRxStat.LengthMismatchCount_B0);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.LengthMismatchCount_B1 : 0x%08x\n", g_HqaRxStat.LengthMismatchCount_B1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.MAC_FCS_Err : 0x%08x\n", g_HqaRxStat.MAC_FCS_Err);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.MAC_FCS_Err1 : 0x%08x\n", g_HqaRxStat.MAC_FCS_Err1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.MAC_Mdrdy : 0x%08x\n", g_HqaRxStat.MAC_Mdrdy);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.MAC_Mdrdy1 : 0x%08x\n", g_HqaRxStat.MAC_Mdrdy1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.MRURxCount : 0x%08x\n", g_HqaRxStat.MRURxCount);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.Noise_Floor : 0x%08x\n", g_HqaRxStat.Noise_Floor);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_FCS_Err_Band1 : 0x%08x\n", g_HqaRxStat.OFDM_FCS_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_PD : 0x%08x\n", g_HqaRxStat.OFDM_PD);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_PD_Band1 : 0x%08x\n", g_HqaRxStat.OFDM_PD_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_SIG_Err : 0x%08x\n", g_HqaRxStat.OFDM_SIG_Err);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_SIG_Err_Band1 : 0x%08x\n", g_HqaRxStat.OFDM_SIG_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_TAG_Err : 0x%08x\n", g_HqaRxStat.OFDM_TAG_Err);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OFDM_TAG_Err_Band1 : 0x%08x\n", g_HqaRxStat.OFDM_TAG_Err_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OutOfResource : 0x%08x\n", g_HqaRxStat.OutOfResource);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.OutOfResource1 : 0x%08x\n", g_HqaRxStat.OutOfResource1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PER0 : 0x%08x\n", g_HqaRxStat.PER0);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PER1 : 0x%08x\n", g_HqaRxStat.PER1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PhyMdrdyCCK : 0x%08x\n", g_HqaRxStat.PhyMdrdyCCK);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PhyMdrdyOFDM : 0x%08x\n", g_HqaRxStat.PhyMdrdyOFDM);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PHY_CCK_MDRDY_Band1 : 0x%08x\n", g_HqaRxStat.PHY_CCK_MDRDY_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PHY_Mdrdy : 0x%08x\n", g_HqaRxStat.PHY_Mdrdy);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.PHY_OFDM_MDRDY_Band1 : 0x%08x\n", g_HqaRxStat.PHY_OFDM_MDRDY_Band1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.RCPI0 : 0x%08x, RCPI1 : 0x%08x, RCPI2 : 0x%08x, RCPI3 : 0x%08x\n",
		g_HqaRxStat.RCPI0, g_HqaRxStat.RCPI1, g_HqaRxStat.RCPI2, g_HqaRxStat.RCPI3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.RSSI0 : 0x%08x, RSSI1 : 0x%08x, RSSI2 : 0x%08x, RSSI3 : 0x%08x\n",
		g_HqaRxStat.RSSI0, g_HqaRxStat.RSSI1, g_HqaRxStat.RSSI2, g_HqaRxStat.RSSI3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.RXOK0 : 0x%08x, RXOK1 : 0x%08x\n", g_HqaRxStat.RXOK0, g_HqaRxStat.RXOK1);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.RXVRSSI : 0x%08x\n", g_HqaRxStat.RXVRSSI);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.SIGMCS : 0x%08x\n", g_HqaRxStat.SIGMCS);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.SINR : 0x%08x\n", g_HqaRxStat.SINR);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.SNR0 : 0x%08x, SNR1 : 0x%08x, SNR2 : 0x%08x, SNR3 : 0x%08x\n",
		g_HqaRxStat.SNR0, g_HqaRxStat.SNR1, g_HqaRxStat.SNR2, g_HqaRxStat.SNR3);
	DBGLOG(RFTEST, TRACE, " g_HqaRxStat.WB_RSSI0 : 0x%08x, WB_RSSI1 : 0x%08x\n",
		g_HqaRxStat.WB_RSSI0, g_HqaRxStat.WB_RSSI1);
					...
					IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DoCalibrationTestItem item : 0x%08x\n", u4Item);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_DoCalibrationTestItem band_idx : %d\n", u4Band_idx);
...
static INT_32 HQA_TMRSetting(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_TMRSetting u4Setting : %d\n", u4Setting);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_TMRSetting u4Version : %d\n", u4Version);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_TMRSetting u4MPThres : %d\n", u4MPThres);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_TMRSetting u4MPIter : %d\n", u4MPIter);
...
/*	PARAM_CUSTOM_EFUSE_BUFFER_MODE_T rSetEfuseBufModeInfo; */
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_WriteBufferDone Value : %d\n", Value);
 ...
 */
	...
	ChipId = 0x00006631;
#else
	/* Workaround for QA tool chip ID check */
#if defined(_HIF_USB)
	rMcrInfo.u4McrOffset = TOP_HCR;
#endif
	rMcrInfo.u4McrData = 0;
	rMcrInfo.ucMcrAntIdx = 0;
	prGlueInfo = *((P_GLUE_INFO_T *) netdev_priv(prNetDev));

	i4Ret = kalIoctl(prGlueInfo, wlanoidQueryMcrRead, &rMcrInfo, sizeof(rMcrInfo), TRUE, TRUE, TRUE, &u4BufLen);

	if (i4Ret == 0)
		ChipId = rMcrInfo.u4McrData;
#endif
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetSeqData u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0 ; i < u4Len ; i++) {
		...
		DBGLOG(RFTEST, INFO,
			" QA_AGENT HQA_MPSSetSeqData mps_setting Case %u (Mode : %lu / TX Path : %lu / MCS : %lu)\n",
			i,
				      ...
				      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPayloadLength u4Band_idx : %d\n", u4Band_idx);
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPacketCount u4Band_idx : %d\n", u4Band_idx);
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPowerGain u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0 ; i < u4Len ; i++) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPowerGain mps_setting Case %d (Power : %d)\n",
			i, mps_setting[i]);
	}
...
static INT_32 HQA_MPSSetNss(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetNss u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0; i < u4Len; i++) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetNss mps_setting Case %d (Nss : %d)\n",
			i, mps_setting[i]);
	}
	...
	HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPerpacketBW u4Band_idx : %d\n", u4Band_idx);
	...
	for (i = 0 ; i < u4Len ; i++) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_MPSSetPerpacketBW mps_setting Case %d (BW : %d)\n",
			i, mps_setting[i]);
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetBandMode u4Band_mode : %d\n", u4Band_mode);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_SetBandMode u4Band_type : %d\n", u4Band_type);
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_GetBandMode u4Band_idx : %d\n", u4Band_idx);
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_GetBandMode g_DBDCEnable = %d\n", g_DBDCEnable);
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (!prInBuf) {
		DBGLOG(RFTEST, ERROR, "allocate memory failed\n");
		return -1;
	}
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_BssInfoUpdate OwnMacIdx : %d\n", OwnMacIdx);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_BssInfoUpdate BssIdx : %d\n", BssIdx);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_BssInfoUpdate addr1:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5]);

	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		OwnMacIdx, BssIdx, ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5]) < 0) {
		kfree(prInBuf);
		return -1;
	}
				...
				IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		OwnMacIdx, ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5], Band) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
static INT_32 HQA_LogOnOff(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_LogOnOff band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_LogOnOff log_type : %d\n", u4Log_type);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_LogOnOff log_ctrl : %d\n", u4Log_ctrl);
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_LogOnOff log_size : %d\n", u4Log_size);
					...
					IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", invalid) < 0) {
		kfree(prInBuf);
		return -1;
	}
					...
					IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", pfmuidx) < 0) {
		kfree(prInBuf);
		return -1;
	}
				       ...
				       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", bftype) < 0) {
		kfree(prInBuf);
		return -1;
	}
				   ...
				   IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", tag_bw) < 0) {
		kfree(prInBuf);
		return -1;
	}
				     ...
				     IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", su_mu) < 0) {
		kfree(prInBuf);
		return -1;
	}
					 ...
					 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		col_idx0, row_idx0, col_idx1, row_idx1, col_idx2, row_idx2, col_idx3, row_idx3) < 0) {
		kfree(prInBuf);
		return -1;
	}
				       ...
				       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x",
		nrow, ncol, ngroup, LM, code_book, htc_exist) < 0) {
		kfree(prInBuf);
		return -1;
	}
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x", snr_sts0, snr_sts1, snr_sts2, snr_sts3) < 0) {
		kfree(prInBuf);
		return -1;
	}
				       ...
				       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", smt_ant) < 0) {
		kfree(prInBuf);
		return -1;
	}
				      ...
				      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", se_idx) < 0) {
		kfree(prInBuf);
		return -1;
	}
					 ...
					 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", rmsd_thrd) < 0) {
		kfree(prInBuf);
		return -1;
	}
					...
					IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x", mcs_lss0, mcs_sss0, mcs_lss1, mcs_sss1, mcs_lss2,
		mcs_sss2) < 0) {
		kfree(prInBuf);
		return -1;
	}
					...
					IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%x", bf_tout) < 0) {
		kfree(prInBuf);
		return -1;
	}
					  ...
					  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", desire_bw) < 0) {
		kfree(prInBuf);
		return -1;
	}
					  ...
					  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", desire_nc) < 0) {
		kfree(prInBuf);
		return -1;
	}
					  ...
					  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", desire_nr) < 0) {
		kfree(prInBuf);
		return -1;
	}
				     ...
				     IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x", idx, isBFer) < 0) {
		kfree(prInBuf);
		return -1;
	}
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	i4Ret = kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		   wlan_idx, bss_idx, aid, mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);

	if (i4Ret < 0) {
		DBGLOG(RFTEST, ERROR, "sprintf failed\n");
		kfree(prInBuf);
		return -1;
	}
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	for (subcarrIdx = subcarr_start; subcarrIdx <= subcarr_end; subcarrIdx++) {
		...
		kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
		if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x", idx, fgBFer, SubIdx[1], SubIdx[0]) < 0) {
			kfree(prInBuf);
			return -1;
		}
				     ...
				     IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf,
		...
		phi22, psi32, phi32, psi42, phi33, psi43, snr00, snr01, snr02, snr03) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
static INT_32 HQA_BFSounding(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		su_mu, mu_num, snd_interval, wlan_id0, wlan_id1, wlan_id2, wlan_id3) < 0) {
		kfree(prInBuf);
		return -1;
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x",
		wlan_id, eBF_enable, iBF_enable, MuTx_enable) < 0) {
		kfree(prInBuf);
		return -1;
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf,
		...
		phymode, bw, nss, pfmuid, marate_mode, marate_mcs, spe_idx, aid, 0) < 0) {
		kfree(prInBuf);
		return -1;
	}
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", u4Gid) < 0) {
		kfree(prInBuf);
		return -1;
	}
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x",
		u4Num_of_user, u4Bandwidth, u4Nss_of_user0, u4Nss_of_user1, u4Pf_mu_id_of_user0, u4Pf_mu_id_of_user1,
		u4Num_of_txer, u4Spe_index, u4Group_index) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
static INT_32 HQA_MUGetLQ(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	INT_32 i4Ret = 0;
	UINT_32 i;
	UINT_8 u4LqReport[NUM_OF_USER * NUM_OF_MODUL] = {0};
	UINT_8 *prInBuf;
				 ...
				 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x", u4Offset) < 0) {
		kfree(prInBuf);
		return -1;
	}
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x", u4Zero_nss) < 0) {
		kfree(prInBuf);
		return -1;
	}
				 ...
				 IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x", u4SpeedUpLq) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
static INT_32 HQA_MUSetGroup(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf,
		...
		ucAddr2[2], ucAddr2[3], ucAddr2[4], ucAddr2[5]) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
static INT_32 HQA_MUGetQD(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf, "%u", u4SubIdx) < 0) {
		kfree(prInBuf);
		return -1;
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x", u4Enable) < 0) {
		kfree(prInBuf);
		return -1;
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	kalMemSet(prInBuf, 0, sizeof(UINT_8) * (HQA_BF_STR_SIZE));
	if (kalSprintf(prInBuf, "%02x:%02x:%02x:%02x:%02x:%02x", au4Gid[0], au4Gid[1], au4Up[0], au4Up[1], au4Up[2],
		au4Up[3]) < 0) {
		kfree(prInBuf);
		return -1;
	}
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	if (kalSprintf(prInBuf,
		...
		ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5],
		ucAddr2[0], ucAddr2[1], ucAddr2[2], ucAddr2[3], ucAddr2[4], ucAddr2[5]) < 0) {
		kfree(prInBuf);
		return -1;
	}
...
/*	UINT_32 u4TriggerMac = 0; */	/* Temp unused */
	...
	if (u4Control == 1 && u4Trigger == 1) {
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_CapWiFiSpectrum u4Trigger=%u, u4RingCapEn=%u, u4TriggerEvent=%u\n",
			u4Trigger, u4RingCapEn, u4TriggerEvent);
		DBGLOG(RFTEST, INFO, " QA_AGENT u4CaptureNode=%u, u4CaptureLen=%u, u4CapStopCycle=%u, u4BW=%u\n",
			u4CaptureNode, u4CaptureLen, u4CapStopCycle, u4BW);

		memset(&g_au4I0Data[0][0], 0, 408000);/* ming-chih add */
		memset(&g_au4Q0Data[0][0], 0, 408000);/* ming-chih add */

		/* iwpriv wlan205 set_test_cmd 75 0   (J mode Setting) */
		rRfATInfo.u4FuncIndex = RF_AT_FUNCID_SET_J_MODE;
		rRfATInfo.u4FuncData = 0;

		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_J_MODE u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_CBW u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_ADC_CLK_MODE u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_ICAP_TRIGGER_OFFSET u4FuncData = %d\n",
					 rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_ICAP_SIZE u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		if (u4CaptureNode == 0x6)
			u4CaptureNode = ICAP_CONTENT_ADC;/*0;*//*0x10000006;*/
		else if (u4CaptureNode == 0x8)
			u4CaptureNode = ICAP_CONTENT_FIIQ;/*2;*//*0x49;*/
		else if (u4CaptureNode == 0x9)
			u4CaptureNode = ICAP_CONTENT_FDIQ;/*3;*//*0x48;*/

		rRfATInfo.u4FuncIndex = RF_AT_FUNCID_SET_ICAP_CONTENT;
		rRfATInfo.u4FuncData = u4CaptureNode;

		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_ICAP_CONTENT u4FuncData = %d\n",
					 rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_ICAP_MODE u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_SET_ICAP_RING u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " Index = RF_AT_FUNCID_COMMAND u4FuncData = %d\n", rRfATInfo.u4FuncData);
		...
		DBGLOG(RFTEST, INFO, " QA_AGENT HQA_CapWiFiSpectrum u4WFNum = %u, u4IQ = %u\n", u4WFNum, u4IQ);
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext central_ch0 : %d\n", u4Central_ch0);
	/* for BW80+80 */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext central_ch1 : %d\n", u4Central_ch1);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext sys_bw : %d\n", u4Sys_bw);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext perpkt_bw : %d\n", u4Perpkt_bw);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext pri_sel : %d\n", u4Pri_sel);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext reason : %d\n", u4Reason);
	/* 0:2.4G    1:5G */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_channel_ext ch_band : %d\n", u4Ch_band);
				    ...
				    IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext band_idx : %d\n", u4Band_idx);
	/* Frame Control...0800 : Beacon */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext FC : 0x%x\n", u4FC);
	/* Duration....NAV */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext dur : 0x%x\n", u4Dur);
	/* Sequence Control */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext seq : 0x%x\n", u4Seq);
	/* Normal:0,Repeat:1,Random:2 */
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext gen_payload_rule : %d\n", u4Gen_payload_rule);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext txlen : %d\n", u4Txlen);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext payload_len : %d\n", u4Payload_len);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext addr1:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr1[0], ucAddr1[1], ucAddr1[2], ucAddr1[3], ucAddr1[4], ucAddr1[5]);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext addr2:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr2[0], ucAddr2[1], ucAddr2[2], ucAddr2[3], ucAddr2[4], ucAddr2[5]);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_set_txcontent_ext addr3:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucAddr3[0], ucAddr3[1], ucAddr3[2], ucAddr3[3], ucAddr3[4], ucAddr3[5]);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext payload : 0x%x\n", ucPayload);
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext pkt_cnt : %d\n", u4Pkt_cnt);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext phymode : %d\n", u4Phymode);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext rate : %d\n", u4Rate);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext pwr : %d\n", u4Pwr);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext stbc : %d\n", u4Stbc);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext ldpc : %d\n", u4Ldpc);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext ibf : %d\n", u4iBF);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext ebf : %d\n", u4eBF);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext wlan_id : %d\n", u4Wlan_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext aifs : %d\n", u4Aifs);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext gi : %d\n", u4Gi);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_tx_ext tx_path : %d\n", u4Tx_path);
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_rx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_rx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_rx_ext band_idx : %d\n", u4Band_idx);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_rx_ext own_mac:%02x:%02x:%02x:%02x:%02x:%02x\n",
	       ucOwn_mac[0], ucOwn_mac[1], ucOwn_mac[2], ucOwn_mac[3], ucOwn_mac[4], ucOwn_mac[5]);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_start_rx_ext rx_path : 0x%x\n", u4Rx_path);
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_tx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_tx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_tx_ext band_idx : %d\n", u4Band_idx);
			      ...
			      IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_rx_ext ext_id : %d\n", u4Ext_id);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_rx_ext param_num : %d\n", u4Param_num);
	DBGLOG(RFTEST, INFO, " QA_AGENT hqa_stop_rx_ext band_idx : %d\n", u4Band_idx);
...
static INT_32 HQA_IRRSetADC(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	/* RunType : 0 -> QA, 1 -> ATE */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetADC u4RunType : %d\n", u4RunType);
	/* FType : 0 -> FI, 1 -> FD */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetADC u4FType : %d\n", u4FType);
			       ...
			       IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain ext_id : %d\n", u4Ext_id);
	/* PGA is for MT663, LPFG is for MT7615 */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain u4PgaLpfg : %d\n", u4PgaLpfg);
	/* 5 : UH, 4 : H, 3 : M, 2 : L, 1 : UL */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain u4Lna : %d\n", u4Lna);
	/* DBDC band0 or band1 */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain u4Band : %d\n", u4Band);
	/* (each bit for each WF) */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain u4WF_inx : 0x%x\n", u4WF_inx);
	/* only for MT6632 */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetRxGain u4Rfdgc : %d\n", u4Rfdgc);
...
static INT_32 HQA_IRRSetTTG(struct net_device *prNetDev, IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTTG u4TTGPwrIdx : %d\n", u4TTGPwrIdx);
	/* Ch Freq: channel frequency value */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTTG u4ChFreq : %d\n", u4ChFreq);
	/* FI Tone Freq(float): driver calculate TTG Freq(TTG Freq = Ch_freq + FI tone freq) */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTTG u4FIToneFreq : %d\n", u4FIToneFreq);
				  ...
				  IN union iwreq_data *prIwReqData, HQA_CMD_FRAME *HqaCmdFrame)
{
	...
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTrunOnTTG ext_id : %u\n", u4Ext_id);
	/* TTG on/off:  0:off,   1: on */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTrunOnTTG u4TTGOnOff : %u\n", u4TTGOnOff);
	/* Band: DBDC band0 or band1 */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTrunOnTTG u4Band : %u\n", u4Band);
	/* (each bit for each WF) */
	DBGLOG(RFTEST, INFO, " QA_AGENT HQA_IRRSetTrunOnTTG u4WF_inx : %u\n", u4WF_inx);
